...
import { FileText, Users, LayoutDashboard, Search, Bell, Box, Activity, UserCircle } from 'lucide-react';
    ...
    <div className="w-full h-full bg-background rounded-xl border border-border shadow-2xl flex flex-col overflow-hidden relative">
      ...
      <div className="flex flex-1 overflow-hidden relative">
        ...
        <div className="w-16 md:w-48 bg-surface/50 border-r border-border p-4 hidden sm:flex flex-col gap-5 z-20">
          <div className="text-xs font-bold text-foreground/40 uppercase tracking-widest mb-2 hidden md:block">Main Menu</div>
          {[
            ...
            <div key={i} className={`flex items-center gap-3 transition-colors ${item.active ? 'opacity-100 text-accent font-medium' : 'opacity-60 hover:opacity-100 text-foreground'}`}>
              ...
              {type === 'pipeline' && (
                <>
                  <Cursor animation={{
                    ...
                    transition: { duration: 10, times: [0, 0.05, 0.1, 0.3, 0.35, 0.45, 0.9, 1], repeat: Infinity }
                  }} />
                  <div className="flex flex-col h-full gap-4 relative">
                    ...
                    <div className="flex gap-4 h-full relative">
                      ...
                      <div className="flex-1 bg-surface/30 rounded-lg p-3 border border-border/50">
                        <div className="text-sm font-semibold text-foreground/80 mb-4 flex justify-between">
                          Negotiation <span className="bg-foreground/10 px-2 rounded-full text-xs">2</span>
                        </div>
                        <div className="w-full bg-surface border border-border rounded-md p-3 shadow-sm mb-3">
                          <div className="font-semibold text-sm text-foreground">Stark Industries</div>
                        ...
                        <motion.div 
                          animate={{ 
                            ...
                            boxShadow: ["none", "0px 10px 20px rgba(0,0,0,0.2)", "0px 10px 20px rgba(0,0,0,0.2)", "0px 5px 15px rgba(34,197,94,0.2)", "0px 5px 15px rgba(34,197,94,0.2)", "none", "none", "none"],
                            borderColor: ["rgba(184,115,51,0.4)", "rgba(184,115,51,1)", "rgba(184,115,51,1)", "rgba(34,197,94,1)", "rgba(34,197,94,1)", "rgba(34,197,94,0)", "rgba(184,115,51,0)", "rgba(184,115,51,0)"]
                          }} 
                          transition={{ duration: 10, times: [0, 0.1, 0.3, 0.35, 0.55, 0.65, 0.9, 1], repeat: Infinity }} 
                      ...
                      <div className="flex-1 bg-surface/30 rounded-lg p-3 border border-border/50 relative">
                        <div className="text-sm font-semibold text-green-500 mb-4 flex justify-between">
                          Closed Won <span className="bg-green-500/10 px-2 rounded-full text-xs">1</span>
                        </div>
                        <div className="w-full bg-surface border border-border rounded-md p-3 shadow-sm mb-3">
                          <div className="font-semibold text-sm text-foreground">Wayne Enterprises</div>
                    ...
                    {/* Success Toast */}
                    <motion.div
                      animate={{ opacity: [0, 0, 0, 1, 1, 0, 0, 0], y: [20, 20, 20, 0, 0, 20, 20, 20] }}
                      transition={{ duration: 10, times: [0, 0.3, 0.35, 0.4, 0.6, 0.65, 0.9, 1], repeat: Infinity }}
              ...
              {type === 'party' && (
                <>
                  ...
                  <div className="flex flex-col h-full gap-6">
                    ...
                    <motion.div 
                      animate={{ backgroundColor: ["rgba(255,255,255,0)", "rgba(255,255,255,0)", "rgba(184,115,51,0.1)", "rgba(184,115,51,0)", "rgba(184,115,51,0)"] }}
                      transition={{ duration: 4.5, times: [0, 0.25, 0.3, 0.5, 1], repeat: Infinity }}
                      className="flex items-center gap-4 border border-border rounded-xl p-4 transition-colors"
                    >
                      <div className="w-12 h-12 rounded-full bg-accent/20 flex items-center justify-center text-accent"><UserCircle className="w-6 h-6" /></div>
                      <div>
                        <div className="font-bold text-lg text-foreground">Acme Corporation</div>
                        <div className="text-xs text-foreground/60">Client since 2021 • B2B Enterprise</div>
                      </div>
                    </motion.div>
                    ...
                    <motion.div 
                      ...
                      transition={{ duration: 4.5, times: [0, 0.3, 0.35, 0.5, 1], repeat: Infinity }}
                      className="grid grid-cols-2 gap-4 overflow-hidden"
                    >
                       <div className="bg-surface/40 border border-border rounded-lg p-4">
                         <div className="text-sm font-semibold text-foreground/80 mb-4">Financial Overview</div>
                         <div className="space-y-3 text-sm">
                           <div className="flex justify-between"><span className="text-foreground/60">Total Lifetime Value</span><span className="font-bold">₹45,50,000</span></div>
                           <div className="flex justify-between"><span className="text-foreground/60">Pending Invoices</span><span className="font-bold text-red-400">₹2,00,000</span></div>
                         </div>
                       </div>
                       <div className="bg-surface/40 border border-border rounded-lg p-4 relative">
                         <div className="text-sm font-semibold text-accent mb-4">Activity Timeline</div>
                         ...
                         <div className="pl-6 space-y-4 relative text-sm">
                           <div className="flex items-start gap-2 relative">
                             ...
                             <div>
                               <div className="text-foreground font-medium">Deal Won: Q3 Licenses</div>
                  ...
                  <div className="flex gap-4 h-full relative">
                    <div className="w-1/3 flex flex-col gap-3 pt-6">
                      ...
                      <div className="w-full bg-surface border border-border rounded flex flex-col p-3 gap-1">
                        ...
                        <div className="flex items-center gap-2"><Box className="w-4 h-4 text-accent/50" /><span className="text-sm">Enterprise License</span></div>
                      </div>
                      <div className="w-full bg-surface border border-border rounded flex flex-col p-3 gap-1">
                        <div className="text-xs text-foreground/50">Item 2</div>
                        <div className="flex items-center gap-2"><Box className="w-4 h-4 text-accent/50" /><span className="text-sm">Support SLA (1 Yr)</span></div>
                      </div>
                      
                      {/* Generate Button being clicked */}
                      <motion.div 
                        animate={{ scale: [1, 1, 0.95, 1, 1], backgroundColor: ["rgba(184,115,51,0.1)", "rgba(184,115,51,0.1)", "rgba(184,115,51,0.3)", "rgba(184,115,51,0.1)", "rgba(184,115,51,0.1)"] }}
                        transition={{ duration: 4.5, times: [0, 0.25, 0.3, 0.4, 1], repeat: Infinity }}
                    ...
                    <div className="flex-1 overflow-hidden relative pt-6">
                      <motion.div 
                        ...
                        animate={{ x: ['100%', '100%', '0%', '0%', '0%'], opacity: [0, 0, 1, 1, 1] }}
                        transition={{ duration: 4.5, times: [0, 0.3, 0.5, 0.9, 1], repeat: Infinity }}
                      ...
                      >
                        ...
                        <div className="relative z-10 flex-grow">
                          <div className="flex justify-between items-start border-b border-border pb-4 mb-4">
                            <div>
                              <div className="font-bold text-lg tracking-widest text-foreground">QUOTATION</div>
                              <div className="text-xs text-foreground/50 mt-1">To: Acme Corporation</div>
                            </div>
                            <div className="bg-accent/20 border border-accent/40 rounded text-accent text-[10px] px-2 py-1 font-mono uppercase">Generated</div>
                          </div>
                          
                          <div className="space-y-2 mb-8">
                             <div className="flex justify-between p-2 bg-background/50 rounded border border-border">
                               <span>Enterprise License (x5)</span><span>₹4,00,000</span>
                             </div>
                             <div className="flex justify-between p-2 bg-background/50 rounded border border-border">
                               <span>Support SLA (1 Yr)</span><span>₹50,000</span>
                             </div>
                          </div>
                        </div>
                ...
                <>
                  ...
                  <div className="flex flex-col h-full gap-4 relative">
                    <div className="flex justify-between items-center mb-2">
                       ...
                       <motion.div 
                          animate={{ scale: [1, 1, 0.9, 1, 1], backgroundColor: ["rgba(0,0,0,0)", "rgba(0,0,0,0)", "rgba(184,115,51,0.2)", "rgba(0,0,0,0)", "rgba(0,0,0,0)"] }}
                          transition={{ duration: 4.5, times: [0, 0.25, 0.3, 0.4, 1], repeat: Infinity }}
                    ...
                    <div className="grid grid-cols-3 gap-4">
                      {[
                        ...
                        <div key={i} className="bg-surface/40 border border-border rounded-lg p-4 flex flex-col gap-1">
                    ...
                    <div className="flex-1 flex gap-4 mt-2">
                      <div className="flex-[2] bg-surface/30 border border-border rounded-lg p-4 flex flex-col justify-end gap-2 overflow-hidden relative">
                         <div className="absolute top-4 left-4 text-xs font-semibold text-foreground/50">Revenue Growth (Q3)</div>
                         <div className="flex items-end justify-between gap-2 h-32 w-full mt-4">
                           {[30, 45, 25, 60, 40, 75, 55].map((h, i) => (
                             <motion.div 
                               key={i} 
                               animate={{ height: ['0%', '0%', '0%', `${h}%`, `${h}%`] }} 
                               transition={{ duration: 4.5, times: [0, 0.3, 0.4, 0.7, 1], ease: "easeOut", repeat: Infinity }} 
    ...
    <section className="py-32 bg-background relative overflow-hidden border-y border-border">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="text-center mb-24">
          ...
          <h2 className="text-4xl md:text-5xl font-bold tracking-tight mb-6 text-foreground max-w-4xl mx-auto">{headline}</h2>
          ...
          <div className="lg:col-span-5 flex flex-col gap-6 relative">
             ...
             {flowSteps.map((step: FlowStep, idx: number) => {
               ...
               return (
                 ...
                 >
                   <div className={`hidden lg:flex absolute left-4 top-1/2 -translate-y-1/2 -ml-2.5 w-5 h-5 rounded-full border-4 items-center justify-center transition-colors duration-300 z-10 ${
                     isActive ? 'border-accent bg-background' : 'border-border bg-background'
                   }`}>
                     {isActive && (
                       <motion.div layoutId="activeStepNode" className="w-2 h-2 rounded-full bg-accent" />
                     )}
                   </div>

                   <div className={`p-6 rounded-2xl border transition-all duration-300 ${
                     isActive ? 'bg-surface/80 border-accent/30 shadow-[0_0_30px_rgba(184,115,51,0.1)]' : 'bg-transparent border-transparent'
                   }`}>
                     <div className="flex items-center gap-4 mb-3">
                       <span className={`font-mono text-sm ${isActive ? 'text-accent' : 'text-foreground/50'}`}>
                         {step.step}
                       </span>
                       <h3 className={`text-xl font-bold ${isActive ? 'text-foreground' : 'text-foreground/70'}`}>
                         {step.title}
                       </h3>
                     </div>
                     <p className={`text-foreground/70 leading-relaxed ${isActive ? 'block' : 'hidden lg:block'}`}>
                       {step.description}
                     </p>
                   </div>
                 </div>
