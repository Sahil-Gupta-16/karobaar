        ...
        <div className="overflow-x-auto pb-8">
          <div className="min-w-[800px] border border-border rounded-2xl bg-surface/30 overflow-hidden relative shadow-2xl">
            ...
            <div className="grid grid-cols-6 border-b border-border bg-background p-4 relative z-10">
              <div className="col-span-2 font-medium text-foreground/70 flex items-center px-4">
                Feature
              </div>
              {competitors.map((comp, idx) => (
                <div
                  ...
                  className={cn(
                    ...
                    comp === "Karobaar"
                      ? "text-accent bg-accent/10 border border-accent/20 shadow-[0_0_20px_rgba(184,115,51,0.1)] text-lg"
                      : "text-foreground",
                  )}
                >
                  {comp}
            ...
            <div className="divide-y divide-border/50 relative z-10 bg-background/50">
              {rows.map((row, idx) => (
                <div
                  key={idx}
                  className="grid grid-cols-6 p-4 items-center hover:bg-surface/50 transition-colors"
                >
                  <div className="col-span-2 font-medium text-foreground/90 px-4">
                    {row.feature}
                  </div>
                        ...
                        {hasFeature ? (
                          <Check
                            className={cn(
                              "w-5 h-5",
                              comp === "Karobaar"
                                ? "text-accent drop-shadow-[0_0_8px_rgba(184,115,51,0.5)]"
                                : "text-foreground/50",
                            )}
                          />
                        ) : (
