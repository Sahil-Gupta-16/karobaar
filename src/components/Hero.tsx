      ...
      <div className="aspect-[4/3] lg:aspect-video w-full rounded-xl overflow-hidden bg-background border border-border flex flex-col relative">
        ...
        <div className="absolute top-0 left-0 w-full h-10 bg-surface border-b border-border flex items-center px-4 gap-2 z-20">
          ...
          <div className="ml-4 flex gap-2">
            {[0, 1, 2, 3].map((i) => (
              <div key={i} className={`h-1.5 w-8 rounded-full transition-colors duration-500 ${step === i ? 'bg-accent' : 'bg-border'}`} />
            ))}
          </div>
        </div>
        ...
        <div className="flex-1 pt-10 relative bg-background/50">
          <AnimatePresence mode="wait">
            ...
            {step === 0 && (
              ...
              >
                <div className="text-sm font-bold text-foreground mb-3 flex items-center gap-2"><ArrowRight className="w-4 h-4 text-accent" /> Sales Pipeline</div>
                <div className="flex gap-3 h-full">
                  ...
                  <div className="flex-1 bg-surface/30 rounded border border-border p-2 relative">
                    ...
                    >
                      ...
                      <div className="text-[10px] text-foreground/60 mt-1">₹12,50,000 • Enterprise</div>
                    </motion.div>
                  </div>
                </div>
                <motion.div 
            ...
            {step === 1 && (
              ...
              >
                <div className="absolute top-4 left-4 text-sm font-bold text-foreground flex items-center gap-2"><FileText className="w-4 h-4 text-accent" /> GST Billing</div>
                ...
                >
                  <div className="flex justify-between border-b border-border pb-2 mb-2">
                    <div>
                      <div className="text-[10px] text-foreground/50 font-bold uppercase">Tax Invoice</div>
            ...
            {step === 2 && (
              <motion.div 
                ...
                className="w-full h-full p-4 flex flex-col"
              >
                <div className="text-sm font-bold text-foreground mb-3 flex items-center gap-2"><Users className="w-4 h-4 text-accent" /> HR & Payroll</div>
                <div className="flex gap-4 mb-4">
                  <div className="flex-1 bg-surface border border-border rounded-lg p-3">
                    <div className="text-[10px] text-foreground/50 uppercase font-bold">Active Employees</div>
                    <div className="text-2xl font-bold text-foreground">54</div>
                  </div>
                  <div className="flex-1 bg-surface border border-border rounded-lg p-3">
                    <div className="text-[10px] text-foreground/50 uppercase font-bold">Payroll Processed</div>
                  ...
                  {[
                    ...
                    >
                      <div className="flex items-center gap-2">
                        <div className="w-6 h-6 rounded-full bg-accent/20 text-accent text-[10px] flex items-center justify-center font-bold">{emp.name[0]}</div>
                        <div>
                          <div className="text-xs font-bold text-foreground">{emp.name}</div>
                          <div className="text-[10px] text-foreground/60">{emp.role}</div>
                        </div>
                      </div>
                      <div className="text-xs font-bold text-green-500 bg-green-500/10 px-2 py-0.5 rounded">{emp.amt}</div>
                    </motion.div>
                  ))}
                </div>
              </motion.div>
            ...
            {step === 3 && (
              ...
              >
                <div className="text-sm font-bold text-foreground mb-4 flex items-center gap-2"><BarChart3 className="w-4 h-4 text-accent" /> Business Overview</div>
                ...
                <div className="flex gap-4">
                  <div className="flex-[2] bg-surface/50 border border-border rounded-lg p-3 h-32 relative overflow-hidden flex flex-col justify-between">
                    <div>
                      <div className="text-[10px] text-foreground/50 uppercase font-bold">Total Revenue</div>
                  ...
                  <div className="flex-1 flex flex-col gap-3">
                    ...
                    >
                      <div className="text-[10px] text-foreground/50 uppercase font-bold">Win Rate</div>
                    ...
                    <motion.div 
                      ...
                      className="bg-surface/50 border border-border rounded-lg p-3 flex-1 flex flex-col justify-center"
                    >
                      <div className="text-[10px] text-foreground/50 uppercase font-bold">Active Deals</div>
      ...
      <div className="w-full px-6 md:px-12 lg:px-20 2xl:px-32 relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-8 items-center">
          ...
          <div className="text-left">
            ...
            >
              <span className="text-xs font-mono text-foreground/80 uppercase tracking-widest font-bold">{kicker}</span>
            </motion.div>
